...
6. **Web-search first for time-sensitive claims.** If the answer depends on current state-of-the-world, WebSearch BEFORE answering. Not "if the user asks" — BEFORE the first sentence. Trusting your weights on current affairs violates rule #1. At session start for any task that could touch the real world, load the search tools eagerly: `ToolSearch select:WebSearch,WebFetch`. They are deferred by default and the loading-on-demand friction is the reason searches get skipped.
...
- **No mocking in tests.** Real services only. Tests must exercise the actual critical path.
